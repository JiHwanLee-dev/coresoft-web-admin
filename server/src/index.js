...
async function getArchievement(res,req){
  try {
    let pool = await sql.connect(config)
    let result1 = await pool.request()
        .query(`SELECT idx, (year + '.' + month) AS date, title, content FROM TB_BUSINESS_ARCHIEVEMENTS WHERE use_yn = 'N' ORDER BY idx DESC`)
...
async function getCompanyHistory(res,req){
  try {
    let pool = await sql.connect(config)
    let result1 = await pool.request()
        .query(`SELECT idx, (year + '.' + month) AS date, title, content FROM TB_COMPANY_HISTORY WHERE use_yn = 'N' ORDER BY idx DESC`)
